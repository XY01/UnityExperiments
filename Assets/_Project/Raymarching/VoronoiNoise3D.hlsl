...
float RayMarchDepth(float3 startPoint, float3 dir, int recursionDepth, float stepDist, float noiseStrength, out float intensity)
{
    float3 currentPoint = startPoint;
    intensity = 0;
    float distance = 0;
    ...
    for (int i = 0; i < recursionDepth; i += 1)
    {
		// Replace v noise with some other smokey noise / 3d tex
        float noise = noise3D(currentPoint) * noiseStrength; //VoronoiNoise3D(currentPoint);
        intensity += pow(noise, 3);// * noise;		
		// Increment sample point
        currentPoint += stepDist * dir;		
    }
	
    //intensity /= recursionDepth;
    return intensity;
