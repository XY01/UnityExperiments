...
namespace Entropedia
{
    ...
    public class Sun : MonoBehaviour
    {
        ...
        {
            // testin hard set year month day
            date =  new DateTime(year, month, day, hour, minutes, 0);
        ...
        void SetPosition()
        {
            ...
            float x = SunDist * Mathf.Cos((float)alt) * Mathf.Cos((float)azi);
            float y = SunDist * Mathf.Cos((float)alt) * Mathf.Sin((float)azi);
            float z = SunDist * Mathf.Sin((float)alt);
            SunTransform.position = new Vector3(x, y, z);
